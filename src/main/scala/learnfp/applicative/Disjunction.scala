...
object DisjunctionInstance {
  implicit def disjunctionInstance[L]: Applicative[Disjunction[L, _]] = new Applicative[({ type E[X] = Disjunction[L, X] })#E]() {
  ...
  implicit def disjunctionToApplicativeOps[L, A, R](fx: Disjunction[L, A => R])(
      implicit applicative: Applicative[({ type E[X] = Disjunction[L, X] })#E]): FxApplicativeOps[A, R, Disjunction[L, _]] =
