...
object Free {
  implicit def freeFunctorInstance[F[_]]: Functor[({ type E[X] = Free[F, X] })#E] = new Functor[({ type E[X] = Free[F, X] })#E] {
    override def fmap[A, B](a: Free[F, A])(fx: A => B): Free[F, B] = ???
  }

  implicit def freeToFunctorOps[F[_], A](a: Free[F, A]): FunctorOps[A, ({ type E[X] = Free[F, X] })#E] =
    new FunctorOps[A, ({ type E[X] = Free[F, X] })#E](a)

  implicit def freeMonadInstance[F[_]]: Monad[({ type E[X] = Free[F, X] })#E] = new Monad[({ type E[X] = Free[F, X] })#E] {
    override def pure[A](a: A): Free[F, A] = ???
    override def flatMap[A, B](a: Free[F, A])(fx: A => Free[F, B]): Free[F, B] = ???
  }

  implicit def freeToMonadOps[F[_], A](a: Free[F, A]): MonadOps[A, ({ type E[X] = Free[F, X] })#E] =
